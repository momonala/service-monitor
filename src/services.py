...
def parse_memory_bytes(memory: str | None) -> int | None:
    """Parse a systemd memory string like '123.4M' into bytes."""
    if not memory:
        return None
    match = re.match(r"^([\d.]+)\s*([KMGT])?B?$", memory.strip().split()[0], re.IGNORECASE)
    if not match:
        return None
    value = float(match.group(1))
    unit = (match.group(2) or "").upper()
    return round(value * _MEMORY_UNIT_BYTES[unit])


def format_memory_bytes(total_bytes: int) -> str:
    """Format bytes as a compact systemd-style memory string."""
    if total_bytes < 1024:
...
def parse_cpu_seconds(cpu: str | None) -> float | None:
    """Parse a systemd CPU time string like '7min 52.884s' into seconds."""
    if not cpu:
        return None
    total = 0.0
    matched = False
    for value, unit in re.findall(r"(\d+(?:\.\d+)?)\s*(h|min|ms|s)", cpu, re.IGNORECASE):
        matched = True
        amount = float(value)
...
def format_cpu_seconds(total_seconds: float) -> str:
    """Format aggregated CPU seconds to a compact human string."""
    if total_seconds < 1:
        return f"{int(round(total_seconds * 1000))}ms"
    ...
    if not hours and not minutes:
        frac = total_seconds - total_int
        return f"{total_int}s" if frac < 0.05 else f"{total_seconds:.1f}s"
    if not hours and seconds:
        parts.append(f"{seconds}s")
    return " ".join(parts[:2] if hours else parts)
...
def aggregate_project_resources(statuses: list[ServiceStatus]) -> dict[str, ProjectResources]:
    """Sum memory and CPU time across all services in each project group."""
    memory_totals: dict[str, int] = {}
    cpu_totals: dict[str, float] = {}
    project_groups: set[str] = set()

    for status in statuses:
        project_groups.add(status.project_group)
        memory_bytes = parse_memory_bytes(status.memory)
        if memory_bytes is not None:
            memory_totals[status.project_group] = memory_totals.get(status.project_group, 0) + memory_bytes
        cpu_seconds = parse_cpu_seconds(status.cpu)
        if cpu_seconds is not None:
            cpu_totals[status.project_group] = cpu_totals.get(status.project_group, 0) + cpu_seconds

    return {
        project_group: ProjectResources(
            memory=(
                format_memory_bytes(memory_totals[project_group]) if project_group in memory_totals else None
            ),
            cpu=format_cpu_seconds(cpu_totals[project_group]) if project_group in cpu_totals else None,
        )
